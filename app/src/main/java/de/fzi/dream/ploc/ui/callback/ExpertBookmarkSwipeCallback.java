...
/**
 * The ExpertBookmarkSwipeCallback provides the callback functionality to response to the users
 * swipes on {@link CardView}s within the {@link ExpertBookmarkFragment} and its {@link RecyclerView}.
 * It is also responsibly for the layout and style of the background displayed when a card is swiped.
 *
 * @author Felix Melcher
 */
public class ExpertBookmarkSwipeCallback extends ItemTouchHelper.Callback {

    /**
     * Public class identifier tag
     */
    public static final String TAG = ExpertBookmarkSwipeCallback.class.getSimpleName();
     ...
     */
    public ExpertBookmarkSwipeCallback(OnExpertBookmarkCardViewListener l) {
        mListener = l;
    }
    ...
    public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
        mListener.onCardLeftSwipe(((ExpertBookmarkViewHolder) viewHolder).getBookmark(), viewHolder.getAdapterPosition());
    }
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            ...
            if (dX > 0) {
                ...
                icon = getBitmapFromDrawable(recyclerView.getContext(), R.drawable.ic_star_green_24dp);
            } else {
