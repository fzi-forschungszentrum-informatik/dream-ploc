...
/**
 * The RecordPreviewSwipeCallback provides the callback functionality to response to the users
 * swipes on {@link CardView}s within the {@link RecordPreviewFragment} and its {@link RecyclerView}.
 * It is also responsibly for the layout and style of the background displayed when a card is swiped.
 *
 * @author Felix Melcher
 */
public class RecordPreviewSwipeCallback extends ItemTouchHelper.Callback {

    /**
     * Public class identifier tag
     */
    public static final String TAG = RecordPreviewSwipeCallback.class.getSimpleName();

    private OnRecordPreviewCardInteraction mListener;
     ...
     */
    public RecordPreviewSwipeCallback(OnRecordPreviewCardInteraction l) {
        mListener = l;
    }
    ...
    public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
        if (direction == ItemTouchHelper.LEFT) {
            mListener.onCardLeftSwipe(((RecordPreviewViewHolder) viewHolder).getRecordPreview(), viewHolder.getAdapterPosition());
        } else {
            mListener.onCardRightSwipe(((RecordPreviewViewHolder) viewHolder).getRecordPreview(), viewHolder.getAdapterPosition());
        }
    }
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            ...
            if (dX > 0) {
                ...
                icon = getBitmapFromDrawable(recyclerView.getContext(), R.drawable.ic_bookmark_white_24dp);
            } else {
                p.setColor(Color.parseColor("#B00020"));
                ...
                icon = getBitmapFromDrawable(recyclerView.getContext(), R.drawable.ic_thumb_down_white_24dp);
            }
